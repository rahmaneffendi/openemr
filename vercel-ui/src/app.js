(function () {
  ...
  function renderShell() {
    ...
    appView.innerHTML = `
      <aside class="sidebar">
        ...
        <nav class="side-nav" id="side-nav">${navItems.map(([id, label]) => `<button class="side-link ${state.activeView === id ? "active" : ""}" type="button" data-action="go" data-view="${id}"><span class="side-dot"></span>${label}</button>`).join("")}</nav>
      ...
      <section class="workspace">
        <header class="topbar">
          ...
          <button class="ghost-button" type="button" data-action="go" data-view="messages">Messages</button>
  ...
  function dashboardView() {
    return `<section class="hero-panel"><div><span class="eyebrow">Today</span><h1>Clinical operations dashboard</h1><p>Appointments, patient flow, billing status, and clinical reminders in one modern workspace.</p></div><button class="btn btn-primary" data-action="go" data-view="calendar" type="button">Open schedule</button></section><section class="metric-grid">${metric("Appointments", "24", "+6 today")}${metric("Checked in", "8", "3 waiting")}${metric("Open claims", "17", "$8.4k")}${metric("Messages", "12", "4 urgent")}</section><section class="dashboard-grid"><article class="panel wide"><div class="panel-header"><h2>Today schedule</h2><button class="ghost-button" data-action="go" data-view="calendar">View all</button></div>${appointmentList()}</article><article class="panel"><div class="panel-header"><h2>Care reminders</h2></div><ul class="clean-list"><li><strong>Jane Sample</strong><span>Lab follow-up due</span></li><li><strong>John Appleseed</strong><span>Annual wellness reminder</span></li><li><strong>Maria Garcia</strong><span>Portal message waiting</span></li></ul></article></section>`;
  }
  ...
  function metric(label, value, detail) { return `<article class="metric-card"><span>${label}</span><strong>${value}</strong><small>${detail}</small></article>`; }
