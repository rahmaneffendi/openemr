(function () {
  ...
  const appName = config.appName || "MediEMR";
  const environment = config.deploymentEnvironment || "local";
  const backendUrl = config.backendUrl || "";
  const patients = {
    jane: { id: "jane", name: "Maya Hartono", dob: "1984-04-16", age: 42, pid: "1001", phone: "0812-2214-4120", provider: "Dr. Sinta", risk: "Medium", balance: "Rp42.000", avatar: "/assets/patient-maya-hartono.png", providerAvatar: "/assets/doctor-sinta.png" },
    john: { id: "john", name: "Daniel Pratama", dob: "1979-11-02", age: 46, pid: "1002", phone: "0813-9910-1010", provider: "Dr. Mahendra", risk: "Low", balance: "Rp0", avatar: "/assets/patient-maya-hartono.png", providerAvatar: "/assets/doctor-sinta.png" }
  };
  const navItems = [
    ["dashboard", "Dashboard"],
    ["calendar", "Schedule"],
  ...
  function renderShell() {
    ...
    appView.innerHTML = `
      <aside class="sidebar">
        <div class="sidebar-brand"><img src="/assets/icon-MediEMR-removebg-preview.png" alt="${escapeHtml(appName)}"><span>${escapeHtml(appName)}</span></div>
      ...
      <section class="workspace">
        <header class="topbar">
          ...
          <div class="user-chip"><span class="avatar"><img src="/assets/doctor-sinta.png" alt="Dr. Sinta"></span><span>Dr. Sinta</span></div>
  ...
  function dashboardView() {
    return `<section class="future-dashboard"><div class="future-hero"><div><span class="eyebrow">Clinic operations</span><h1>Morning worklist</h1><p>Appointments, open chart tasks, messages, and claims that need staff attention today.</p></div><div class="hero-orbit" aria-hidden="true"><strong>18</strong><small>items due today</small></div></div><section class="dashboard-kpis">${futureKpi("Scheduled visits", "32", "8 checked in", icons.patients)}${futureKpi("Average wait", "14m", "front desk estimate", icons.calendar)}${futureKpi("Open chart tasks", "7", "3 need provider review", icons.reports)}${futureKpi("Claims ready", "72%", "clean claim rate", icons.billing)}</section><section class="future-grid"><article class="panel command-chart"><div class="panel-header"><div><h2>Patient flow</h2><span class="panel-subtitle">Visits by selected time window</span></div>${dashboardRangeControl()}</div>${dashboardFlowChart()}<div class="chart-insights"><span><strong>${dashboardRangeData().peak}</strong> busiest slot</span><span><strong>${dashboardRangeData().capacity}</strong> room use</span><span><strong>${dashboardRangeData().risk}</strong> open tasks</span></div></article><article class="panel ai-panel"><div class="panel-header"><div><h2>Work queue</h2><span class="panel-subtitle">Items waiting for a person</span></div></div><ul class="ai-list"><li><span class="ai-score">1</span><div><strong>Lab result to review</strong><small>Maya Hartono has a result waiting for Dr. Sinta.</small></div></li><li><span class="ai-score">2</span><div><strong>Room 2 may run late</strong><small>The 10:15 visit has not started; front desk can update the patient.</small></div></li><li><span class="ai-score">3</span><div><strong>Diagnosis needed on claim</strong><small>Two claims are missing a final diagnosis confirmation.</small></div></li></ul></article><article class="panel flow-lane"><div class="panel-header"><h2>Today timeline</h2><button class="ghost-button icon-label" data-action="go" data-view="calendar" type="button">${icons.calendar}<span>Schedule</span></button></div><div class="lane-list"><button type="button" data-action="select-patient" data-patient="jane"><time>09:00</time><strong>Maya Hartono</strong><span>Room 2 - Office visit</span></button><button type="button" data-action="select-patient" data-patient="john"><time>10:15</time><strong>Daniel Pratama</strong><span>Room 4 - Follow-up</span></button><button type="button"><time>13:30</time><strong>Care conference</strong><span>Telehealth - Team</span></button></div></article><article class="panel revenue-pulse"><div class="panel-header"><div><h2>Claims status</h2><span class="panel-subtitle">Billing work in progress</span></div></div>${claimsDonutChart()}</article></section></section>`;
  }

  function calendarView() {
    return `<div class="page-title"><div><span class="eyebrow">Schedule</span><h1>Calendar</h1></div><button class="btn btn-primary icon-label" type="button">${icons.plus}<span>New appointment</span></button></div><section class="calendar-modern">${["Mon", "Tue", "Wed", "Thu", "Fri"].map((day, index) => `<article class="day-column"><h2>${day}</h2><button class="appointment-card" type="button" data-action="select-patient" data-patient="${index % 2 ? "john" : "jane"}"><span>${9 + index}:00 AM</span><strong>${index % 2 ? "Daniel Pratama" : "Maya Hartono"}</strong><small>Office visit</small></button><button class="appointment-card muted" type="button"><span>1:30 PM</span><strong>Follow-up</strong><small>Room ${index + 1}</small></button></article>`).join("")}</section>`;
  }
  ...
  function billingView(patient) {
    return `<div class="page-title"><div><span class="eyebrow">Revenue cycle</span><h1>Billing</h1></div><button class="btn btn-primary icon-label" type="button">${icons.plus}<span>Post payment</span></button></div><section class="panel"><table class="data-table"><thead><tr><th>Date</th><th>Patient</th><th>Code</th><th>Description</th><th>Amount</th></tr></thead><tbody><tr><td>2026-06-20</td><td>${patient ? patient.name : "Maya Hartono"}</td><td>99213</td><td>Office outpatient visit</td><td>Rp125.000</td></tr><tr><td>2026-06-20</td><td>${patient ? patient.name : "Maya Hartono"}</td><td>81002</td><td>Urinalysis</td><td>Rp18.000</td></tr></tbody></table></section>`;
  }
  ...
  function reportPatientCard() {
    return `<article class="patient-report-card"><div class="report-person"><div class="patient-avatar-lg"><img src="/assets/patient-maya-hartono.png" alt="Maya Hartono"></div><div><h2>Maya Hartono</h2><span class="soft-pill">Female - 42 y/o</span><dl><dt>Patient ID</dt><dd>P-2026-05821</dd><dt>Date of Birth</dt><dd>Apr 16, 1984</dd></dl></div></div><div class="report-meta-grid"><div class="meta-item"><span class="meta-icon">${icons.reports}</span><div><small>Report Type</small><strong>ECG</strong></div></div><div class="meta-item"><span class="meta-icon">${icons.calendar}</span><div><small>Recorded On</small><strong>May 18, 2026 10:30 AM</strong></div></div><div class="meta-item"><span class="meta-icon">${icons.patients}</span><div><small>Recorded By</small><strong>Nurse Olivia</strong></div></div><div class="meta-item"><span class="provider-photo"><img src="/assets/doctor-sinta.png" alt="Dr. Sinta"></span><div><small>Referring Doctor</small><strong>Dr. Sinta</strong></div></div><div class="meta-item"><span class="meta-icon">${icons.documents}</span><div><small>Department</small><strong>Cardiology</strong></div></div><div class="meta-item"><span class="meta-icon">${icons.check}</span><div><small>Status</small><strong><span class="status-pill">Completed</span></strong></div></div></div></article>`;
  }
  ...
  function appointmentList() {
    return `<div class="timeline"><button type="button" data-action="select-patient" data-patient="jane"><span>09:00</span><strong>Maya Hartono</strong><small>Room 2 - Office Visit</small></button><button type="button" data-action="select-patient" data-patient="john"><span>10:15</span><strong>Daniel Pratama</strong><small>Room 4 - Follow-up</small></button><button type="button"><span>13:30</span><strong>Care conference</strong><small>Telehealth - Team</small></button></div>`;
  }
